...
open(unit=42,action="write",status="replace",file="datuSimulatu.dat")   ! posizioak
open(unit=43,action="write",status="replace",file="datuSimulatu2..dat")  ! dibergentzia, kurbadura, gainazal tentsioa, presioa eta indarrak
open(unit=44,action="write",status="replace",file="datuSimulatu.3.dat")  ! algoritmo 3
open(unit=45,action="write",status="replace",file="datuSimulatu.4.dat") 
...
! definitu hasierako u eta bortizitatea
  ...
  do i=1,i_kopurua
    ...
    u_gorde(i,1)=(random1-0.5_dp)*2.0_dp*abiadura_randomizatu  ! (0,1) izatetik (-0.5,0.5)*2=(-1,1) izatera, pasatu zorizko zenbakia
    call random_number(random1)
    u_gorde(i,2)=(random1-0.5_dp)*2.0_dp*abiadura_randomizatu
    u_gorde(i,3)=0.0_dp   ! =0 planoan gaude hasieran

    call random_number(random2)
    bortizitatea_gorde(i)=(random2-0.5_dp)*2.0_dp*bortizitatea_randomizatu

    h_desplazatua_gorde(i)=h_gorde(i)

    call random_number(random2)
    kontzentrazioa_gorde(i)=kontzentrazioa_0+(random2-0.5_dp)*2.0_dp*kontzentrazioa_randomizatu*kontzentrazioa_0
  enddo
...
! algoritmo 1: prestaketa
  ...
  write(unit=43,fmt="(a,e24.14)")  "konpresibilitate_i--->" ,alpha_h*((h_gorde(i)/hi_0)-1.0_dp)
...
!====-----====
  ...
  !aldatu posizioa
    ...
    call lortu_ez_itsatsi_karratu(f(i,1:3),u_gorde(i,1:3))  ! (pos,ui)
...
enddo!-----partikula guztien gaineko loop-a itxi
  ...
  if (zmin_plot>minval(f(:,3))) then !lortu simulazio osoan zehar z min eta max marrazterako orduan eskala egokia izateko
    zmin_plot=minval(f(:,3))
  endif
  if (zmax_plot<maxval(f(:,3))) then
    zmax_plot=maxval(f(:,3))
  endif
  write(unit=45,fmt="(2f12.7)") zmin_plot,zmax_plot ! gorde balio hauek eskuz erabili nahi izatekotan gero
...
! gnuplot komandoak guztia marrazteko
  ...
  do i=0,t_total-1 !0-tik hasi behar index-erako
    write(unit=52,fmt="(a,i5,a)") 'splot "datuSimulatu..dat" index', i,' using 1:2:3:4 pt 7 ps 1 palette notitle'  ! posizioak marraztu t bakoitzean
    !write(unit=52,fmt="(a,i5,a)") '"datuSimulatu4.dat" index', i,' using 1:2:3:4 pt 7 ps 1 palette notitle'  ! abiaduren moduluak  marraztu t bakoitzean
  enddo

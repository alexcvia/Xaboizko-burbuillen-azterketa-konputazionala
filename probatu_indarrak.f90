...
! lortu mugan dauden partikulen indizeak
  ...
  allocate(muga_indizeak(n*4)) 
  call lortu_muga_indize_karratu(f,n,muga_indizeak) ! (partikulak,n,muga_indizeak)
...
if (size(auzokoen_indizeak,1)>nj_max) then
 write(unit=109,fmt=*) "(9 errorea) arazoa programa nagusian (196 lerroan):"  
...
! algoritmoen 2: indarren kalkulua
  ...
  if (t<=50) then
    if ( (f(i,1)>(luzera_x0*0.45)).and.(f(i,1)<(luzera_x0*0.55)).and.(f(i,2)>(luzera_y0*0.45)).and.(f(i,2)<(luzera_y0*0.55)) ) then
...
! algoritmoen 3: denbora aktualizazioa
  ...
  !aldatu abiadura
    ...
    do k=1,size(muga_indizeak,1)           ! i eta j erabiltzen ari ditugu
      u_gorde(muga_indizeak(k),1:3)=0.0_dp ! muga partikulen abiadura =0 berdefinitu
    enddo

  !aldatu posizioa
    f(i,1:3)=f(i,1:3)+u_gorde(i,1:3)*delta_t
    call lortu_ez_itsatsi_karratu(f(i,1:3),u_gorde(i,1:3))  ! (pos,ui)
    !if ( (f(i,1)<0.0_dp).or.(f(i,2)<0.0_dp).or.(f(i,1)>luzera_x0).or.(f(i,2)>luzera_y0)) then !ez ditugu posizio negatiboak nahi ezta karratutik atera
    !  f(i,1:3)=f(i,1:3)-u_gorde(i,1:3)*delta_t
    !endif
...
! m*2 gauza gorde ditut, auzoko eta bektoreak tartekatzen baitira fitxategian, beraz do-an kontutan hartu
  ...
  write(unit=52,fmt="(a)") 'set xrange [0.0:0.05]'                                                       ! gnuplot prestatu
  write(unit=52,fmt="(a)") 'set yrange [0.0:0.05]'                                                       ! gnuplot prestatu
  write(unit=52,fmt="(a)") 'set zrange [0.015:0.025]'                                                    ! gnuplot prestatu
  write(unit=52,fmt="(a)") 'set cbrange [0.0:0.0005]'                                                      ! 10**-4
  do i=0,t_total-1 !0-tik hasi behar index-erako
    write(unit=52,fmt="(a,i5,a)") 'splot "datuIndarrak.dat" index', i,' using 1:2:3:4 pt 7 ps 1 palette notitle'  ! posizioak marraztu t bakoitzean
    !write(unit=52,fmt="(a,i5,a)") '"datuIndarrak4.dat" index', i,' using 1:2:3:4 pt 7 ps 1 palette notitle'  ! abiaduren moduluak  marraztu t bakoitzean
  enddo

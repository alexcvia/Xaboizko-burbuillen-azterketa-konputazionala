  ...
  do i=1,npartikula
    do j=1,zatiKopurua 
      if (partikulak_sare_indizeekin(i,1)==sare_zati_bilduma(j,1).and.&
        ...
        momentuz_zati_sare_hiztegia(sare_zati_bilduma(j,1),&
                                    sare_zati_bilduma(j,2),&
                                    sare_zati_bilduma(j,3),&
                                    hiztegiIndizeKontadore(j)) = i  !gorde sare zati koordenatu honetan zein partikula dauden
        hiztegiIndizeKontadore(j) = hiztegiIndizeKontadore(j)  + 1  !sare zati koord hauek berriro agertzean hurrengo array indizean gorde
        exit
      endif
    enddo 
  enddo

  indizeMax=maxval(hiztegiIndizeKontadore(:))   !lortu zer zati sare daukan ptukopuru handiena

...
subroutine zati_sarea_sortu(partikula_guztiak,zati_sare_hiztegia)
  ...
  !real(kind=dp)                                     :: zmin  !indize handiegien arazoa konpontzeko
  !integer                                           :: indize_zmin
...
if (partikula_guztiak(i,j)<0) then
 write(unit=109,fmt=*) "(3 errorea) arazoa auzokoak moduluan (118 lerroan):"  
...
if (zatiKopurua>=npartikula) then
 write(unit=109,fmt=*) "(5 errorea) arazoa auzokoak moduluan (178 lerroan):"  
...
if (erroZatiKop**3*erroPartikulaKop>10**9) then
 write(unit=109,fmt=*) "(1 errorea) arazoa auzokoak moduluan (204 lerroan):"  
...
!====debug====
if ( (xhiztegi>erroZatiKop).or.(yhiztegi>erroZatiKop).or.(zhiztegi>erroZatiKop) ) then
 write(unit=109,fmt=*) "(8 errorea) arazoa auzokoak moduluan (237 lerroan):"  
 write(unit=109,fmt=*) "--> koordenatu indize memoria maximoa baino indize gehio gordetzen sahiatu da"
 stop  
endif
!====-----====
...
if (hiztegiIndizeKontadore(xhiztegi,yhiztegi,zhiztegi)>erroPartikulaKop+1) then  ! +1 egin zeren +1 extra bat dago kontadorean
 write(unit=109,fmt=*) "(2 errorea) arazoa auzokoak moduluan (242 lerroan):"  
 write(unit=109,fmt=*) "--> partikula indize memoria maximoa baino indize gehio gordetzen sahiatu da"
 stop  
endif
!====-----====

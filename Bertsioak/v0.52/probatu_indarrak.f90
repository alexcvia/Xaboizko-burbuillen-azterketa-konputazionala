...
! zenbat i_partikulen auzokoak bilatu ezarri eta loopa egin
  ...
  nj_max=floor(sqrt(real(i_kopurua)))*2
...
write(unit=42,fmt=*) 2
  ...
  do i=1,i_kopurua
    ...
    if ( (f(i,1)>(luzera_x0*0.25)).and.(f(i,1)<(luzera_x0*0.75)).and.(f(i,2)>(luzera_y0*0.25)).and.(f(i,1)<(luzera_y0*0.75)) ) then
      u_gorde(i,1)=0.01_dp  ! (0,1) izatetik (-0.5,0.5) izatera pasatu zorizko zenbakia
      u_gorde(i,2)=0.0_dp
...
if (size(auzokoen_indizeak,1)>nj_max) then
 ...
 write(unit=109,fmt=*) "--> auzokoetarako esleitutako memoria baino auzoko gehio daude"
 stop  
endif
!====-----====
!-----------------------------------------------------
...
! m*2 gauza gorde ditut, auzoko eta bektoreak tartekatzen baitira fitxategian, beraz do-an kontutan hartu
  ...
  write(unit=52,fmt="(a,i5,a)") 'set xrange [0.0:0.12]'                                                        ! gnuplot prestatu
  write(unit=52,fmt="(a,i5,a)") 'set yrange [0.0:0.12]'                                                        ! gnuplot prestatu
  write(unit=52,fmt="(a,i5,a)") 'set zrange [0.015:0.025]'                                                        ! gnuplot prestatu
  write(unit=52,fmt="(a,i5,a)") 'set cbrange [0.0:4e-7]'                                                      ! 10**-6

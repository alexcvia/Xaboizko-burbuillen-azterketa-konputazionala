...
echo konpilatzen
ifort -o probatu_prestaketa tipos.f90 parametroak.f90 auzokoak.f90 funtzioak.f90 mcf_matrices.f90 lokala.f90 bektoreak.f90 prestaketa.f90 probatu_prestaketa.f90

rm erroreenLog.dat  #<--- reseteatu erroreenLog-a
...
:wq
EOF
sed -i '/grep -v/a ./probatu_prestaketa' bidali.sh   #<<<----- aldatu bidaliko den lana

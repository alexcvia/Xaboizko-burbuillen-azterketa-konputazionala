...
real(kind=dp), dimension(3)             :: gradientea_kartesiarretan  ! izenak esaten du (probak egiteko da ez dut benetan behar)


open(unit=42,action="write",status="replace",file="datuBektoreak.dat")  ! auzokoak, lokala, eta g
open(unit=43,action="write",status="replace",file="datuBektoreak2.dat") ! Rij
open(unit=44,action="write",status="replace",file="datuBektoreak3.dat") ! Uij
open(unit=45,action="write",status="replace",file="datuBektoreak4.dat") ! gainazal_gradientea
...
! lortu ij tentsore metrikoa
  ...
  write(unit=42,fmt=*) g(1,1:2),"g(1,1:2)"
  write(unit=42,fmt=*) g(2,1:2),"g(2,1:2)"
  write(unit=42,fmt=*) det_g,"det_g"
  write(unit=42,fmt=*) g_1(1,1:2),"g_1(1,1:2)"
  write(unit=42,fmt=*) g_1(2,1:2),"g_1(2,1:2)"
  write(unit=42,fmt=*) det_g_1,"det_g_1"
...
! lortu posizio bektore erlatiboak sistema lokal bakoitzeko
  ...
  do i=1,size(auzokoen_indizeak,1)
    ...
    !gainazal_gradientea=gainazal_gradientea/norm2(gainazal_gradientea)  
    !gainazal_gradientea=-gainazal_gradientea/4.0_dp 
    !gradientea_kartesiarretan(1)=sum((/gainazal_gradientea(1),gainazal_gradientea(2),0.0_dp/)*(/1.0_dp,0.0_dp,0.0_dp/))
    !gradientea_kartesiarretan(2)=sum((/gainazal_gradientea(1),gainazal_gradientea(2),0.0_dp/)*(/0.0_dp,1.0_dp,0.0_dp/))
    !gradientea_kartesiarretan(3)=sum((/gainazal_gradientea(1),gainazal_gradientea(2),0.0_dp/)*(/0.0_dp,0.0_dp,1.0_dp/))
    !gradientea_kartesiarretan=sum(sum((/gainazal_gradientea(1),gainazal_gradientea(2),0.0_dp/)*i_oinarria(1:3,1))*(/1,0,0/))*(/1,0,0/)&
    !                         +sum(sum((/gainazal_gradientea(1),gainazal_gradientea(2),0.0_dp/)*i_oinarria(1:3,2))*(/0,1,0/))*(/0,1,0/)&
    !                         +sum(sum((/gainazal_gradientea(1),gainazal_gradientea(2),0.0_dp/)*i_oinarria(1:3,3))*(/0,0,1/))*(/0,0,1/)
    !write(unit=45,fmt="(6f20.14)")  f(i_partikula,1:2),0.0_dp,gradientea_kartesiarretan(1:3) !2D denez, marrazteko z=0 gehitu (z=0 planoan marraztuko dugu)
  ...
  enddo
    ...
    gradientea=gradientea/norm2(gradientea)  
    write(unit=45,fmt="(6f20.14)")  f(i_partikula,1:3),gradientea(1:2),0.0_dp !2D denez, marrazteko z=0 gehitu (z=0 planoan marraztuko dugu)

...
!write(unit=42,fmt="(a,3f20.14)") "------>ibektore", ibektore(1:3)
!write(unit=42,fmt="(a,3f20.14)") "------>i1", i1(1:3)
!write(unit=42,fmt="(a,3f20.14)") "------>i2", i2(1:3)
!write(unit=42,fmt="(a,3f20.14)") "------>ni", ni(1:3)
...
!write(unit=42,fmt="(a,3f20.14)") "------>jbektore", jbektore(1:3)
...
write(unit=42,fmt="(a,3f20.14)") "------>Rij", Rij(1:3)
write(unit=42,fmt="(a,3f20.14)") "------>Rij_i", Rij_i(1:3)

    E3(i)     =Rij_i(3)
!write(unit=42,fmt=*) "------>E3", E3(i)
...
!do i=1,j_kopurua
!write(unit=42,fmt=*) "------>W(i,i)", W(i,i)
!enddo
!do i=1,j_kopurua
!write(unit=42,fmt="(a,6f20.14)") "------>B(i,1:6)", B(i,1:6)
!enddo
!do i=1,6
!write(unit=42,fmt=*)"------>B(1:6,j)", B_T(i,1:j_kopurua)
!enddo
  ...
  beta=soluzioa(:,1)
write(unit=42,fmt="(a,6f20.14)") "------>beta", beta(1:6)
...
subroutine lortu_Rij_gainazal(ri,rj,ni,Rij_gainazal)
  ...
  !Rij_gainazal=Rij_i
  !Rij_gainazal(3)=Rij_gainazal(3)-Rij_i(3)
...
subroutine lortu_gainazal_gradientea(g_1,Rij_gainazal,iren_oinarria,gainazal_gradientea)
  ...
  real(kind=dp),intent(in),dimension(3,3) :: iren_oinarria ! i_ren_sistema_lokala-ren output-a
  real(kind=dp),intent(out),dimension(2)  :: gainazal_gradientea
...
write(unit=45,fmt="(a,f20.14)") "------>r", r
!write(unit=45,fmt="(2f20.14,a)") Rij_norm, "Rij_norm"

  r1=sum(Rij_norm*i1)/norm2(i1)
  r2=sum(Rij_norm*i2)/norm2(i2)
write(unit=45,fmt="(a,f20.14)") "------>r1", r1
write(unit=45,fmt="(a,f20.14)") "------>r2", r2
  
  k=do_kernel(r,h)
  gainazal_gradientea(1)=g_1(1,1)*r1+g_1(1,2)*r2
  gainazal_gradientea(2)=g_1(2,1)*r1+g_1(2,2)*r2
write(unit=45,fmt="(a,f20.14)") "------>gainazal_gradientea(1) bider k baino lehen", gainazal_gradientea(1)
write(unit=45,fmt="(a,f20.14)") "------>gainazal_gradientea(2) bider k baino lehen", gainazal_gradientea(2)
  !gainazal_gradientea=gainazal_gradientea*k
...
write(unit=45,fmt="(a,f20.14)") "------>gainazal_gradientea(1)", gainazal_gradientea(1)
write(unit=45,fmt="(a,f20.14)") "------>gainazal_gradientea(2)", gainazal_gradientea(2)
!write(unit=45,fmt="(2f20.14,a)") gainazal_gradientea, "gainazal_gradientea"

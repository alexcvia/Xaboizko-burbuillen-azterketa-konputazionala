...
! definitu hasierako u eta bortizitatea
  ...
  do i=1,i_kopurua
    ...
    u_gorde(i,1)=(random1-0.5_dp)*abiadura_randomizatu  ! (0,1) izatetik (-0.5,0.5) izatera pasatu zorizko zenbakia
    call random_number(random1)
    u_gorde(i,2)=(random1-0.5_dp)*abiadura_randomizatu
    u_gorde(i,3)=0.0_dp   ! =0 planoan gaude hasieran

    call random_number(random2)
    bortizitatea_gorde(i)=random2*bortizitatea_randomizatu

    h_desplazatua_gorde(i)=h_gorde(i)

    call random_number(random2)
    kontzentrazioa_gorde(i)=kontzentrazioa_0+(random2-0.5_dp)*kontzentrazioa_randomizatu
  enddo

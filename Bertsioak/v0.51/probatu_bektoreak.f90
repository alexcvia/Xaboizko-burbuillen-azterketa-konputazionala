...
real(kind=dp), dimension(3)             :: gradientea           ! gainazal gradienteen batura (probak egiteko da ez dut benetan behar)
...
! lortu posizio bektore erlatiboak sistema lokal bakoitzeko
  ...
  do i=1,size(auzokoen_indizeak,1)
    ...
    !write(unit=45,fmt="(6f20.14)")  f(i_partikula,1:3),gainazal_gradientea(1:3) ! ez dugu marraztuko balio sorta oso ezberdinak baitira kernelagatik
    gradientea=gradientea+gainazal_gradientea    

  enddo

    !gradientea=gradientea/norm2(gradientea)  
    write(unit=45,fmt="(6f20.14)")  f(i_partikula,1:3),gradientea(1:3) 

...
! algoritmoen 2: indarren kalkulua
  ...
  if (t<=50) then
    if ( (f(i,1)>((luzera_x0+x0)*0.45)).and.(f(i,1)<((luzera_x0+x0)*0.55)).and.(f(i,2)>((luzera_y0+y0)*0.45)).and.(f(i,2)<((luzera_y0+y0)*0.55)) ) then
...
! m*2 gauza gorde ditut, auzoko eta bektoreak tartekatzen baitira fitxategian, beraz do-an kontutan hartu
  ...
  write(unit=52,fmt="(a)") 'set zrange [1.015:1.025]'                                                    ! gnuplot prestatu

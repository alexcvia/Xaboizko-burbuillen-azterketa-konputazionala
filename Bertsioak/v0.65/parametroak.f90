...
!denbora pausua
  ...
  real(kind=dp),parameter::  delta_t       = 0.2_dp                ! 5fps
  !real(kind=dp),parameter::  delta_t       = 0.5_dp                ! 2fps
  integer,parameter      ::  t_total       = 5000                  ! iterazio kopurua

!kalibrazio parametroak
  real(kind=dp),parameter::  alpha_h       = 1.0e+1_dp              ! altuera konpresibilitatea
  real(kind=dp),parameter::  alpha_k       = 1.0e+1_dp              ! kurbaduraren eta gainazal tentsio eragina
  real(kind=dp),parameter::  alpha_d       = 1.0e+1_dp              ! dibergentziaren eragina
  real(kind=dp),parameter::  alpha_c       = 2.3e-9_dp              ! kontzentrazio difusio koefizientea

  !real(kind=dp),parameter::  alpha_b       = 1.0e-1_dp              !bortizitaterako ez dut behar, HB-n dago
  real(kind=dp),parameter::  alpha_m       = 1.0e+0_dp              ! marangoni indarraren eragina 
  real(kind=dp),parameter::  alpha_l       = 1.0e+0_dp              ! kapilare indarraren eragina
  real(kind=dp),parameter::  alpha_b       = 1.0e+5_dp              ! biskositate indarraren eragina

  real(kind=dp),parameter::  ui_max        = (R_auzo/delta_t)/100.0_dp ! abiadura maximinoa, hau gainditzekotan auzokoen bilaketa gaizki (zati 100 a ver)
...
!azelerazio grabitatorioa
  ...
  real(kind=dp),parameter::  grabitate     = -9.81_dp*1.0e-8_dp      ! (m/s**2)
  !real(kind=dp),parameter::  grabitate     = 0.0_dp      ! (m/s**2)
...
!========================================= Hasierako Baldintzak ===========================================================
 !HB kalkulatzeko hazia (seed):
  integer,dimension(2),parameter:: hazia                = (/1447846609,1690709269/) ! ( )
 !atseden xaboi kontzentrazioa:
  real(kind=dp),parameter::  kontzentrazioa_0           = 6.67e-8_dp           ! (mol/m**2)
  real(kind=dp),parameter::  kontzentrazioa_randomizatu = 1e-3_dp              ! ( )
 !atseden altuera:
 ...
 !atseden bortizitatea:
  real(kind=dp),parameter::  bortizitatea_randomizatu   = 1.0e-6_dp            ! ( ) a ojo
 !atseden abiadura:
  real(kind=dp),parameter::  abiadura_randomizatu       = 1.0e-5_dp            ! ( ) a ojo
...
!geometriaren mendeko hasierako balioak
  ...
  !real(kind=dp),parameter::  mi_0          = masa_0/npartikula_0     ! (kg) masa partikulako (4e-13)
...
!kanpo indar extrak
  ...
  real(kind=dp),dimension(3),parameter:: extra_indarra_1    = (/0.0_dp,0.0_dp,0.0_dp/)  ! kanpo indar 1

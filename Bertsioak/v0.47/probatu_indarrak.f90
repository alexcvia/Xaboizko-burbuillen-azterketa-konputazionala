...
real(kind=dp)                             :: bortizitate_indarra_j
real(kind=dp)                             :: bortizitate_indarra_i! i-ren auzokoen bortizitate_indarra_j guztien batura
real(kind=dp)                             :: presio_indarra_j
real(kind=dp)                             :: presio_indarra_i     ! j guztien batura
real(kind=dp)                             :: kapilare_indarra_j
real(kind=dp)                             :: kapilare_indarra_i   ! j guztien batura
real(kind=dp)                             :: biskositate_indarra_j
real(kind=dp)                             :: biskositate_indarra_i! j guztien batura
...
open(unit=43,action="write",status="replace",file="datuPrestaketa2.dat") ! dibergentzia, kurbadura, gainazal tentsioa eta presioa
...
! zenbat i_partikulen auzokoak bilatu ezarri eta loopa egin
  ...
  tentsioa_gorde(:)      =0.0_dp
  u_gorde(:,:)           =0.002_dp

  call zati_sarea_sortu(f,f_sare)
 
!----partikula guztien altuera numerikoa lortu 0. iterazioan --> H.B.
  do i=1,i_kopurua
  h_gorde(i)=0.0_dp
...
! lortu auzokoen zerrenda
  ...
  !write(unit=42,fmt=*)   !ez ditugu marraztuko auzoko guztiak partikula guztientzako
  !write(unit=42,fmt=*)
  !do j=1,size(auzokoen_indizeak,1)
  !  write(unit=42,fmt=*) f(auzokoen_indizeak(j),1:3)
  !enddo
...
! lortu sistema lokalaren oinarri bektoreak
  ...
  !if (mod(real(i),27.0_dp)==0.0_dp)then  
  !write(unit=42,fmt=*)
  !write(unit=42,fmt=*)
  !write(unit=42,fmt="(6f20.14)")  f(i,1:3),i_oinarria(1:3,1)*0.01
  !write(unit=42,fmt="(6f20.14)")  f(i,1:3),i_oinarria(1:3,2)*0.01
  !write(unit=42,fmt="(6f20.14)")  f(i,1:3),i_oinarria(1:3,3)*0.01
  !endif
...
!
  ...
  ! indarrak lortu
    ...
    call lort_Marangoni_indarra_j(     h_gorde(i),h_gorde(j),Vi_0,Vi_0,tentsioa_gorde(i),tentsioa_gorde(j),gainazal_gradientea,Marangoni_indarra_j)
  ...
  ! algoritmo1 bukatu
  call lortu_gainazal_tentsioa_i(kontzentrazioa_gorde(i),gainazal_tentsioa_i)                      !(xaboi_kontzentrazioa_i,gainazal_tentsioa_i)
  call lortu_presioa_i(h_gorde(i),gainazal_tentsioa_i,kurbadura_lokala_i,dibergentzia_i,presioa_i) !(hi,gainazal_tentsioa_i,kurbadura_lokala_i,dibergentzia_i,presioa_i)

...
echo konpilatzen
ifort -o simulatu tipos.f90 parametroak.f90 auzokoak.f90 funtzioak.f90 mcf_matrices.f90 lokala.f90 bektoreak.f90 prestaketa.f90 indarrak.f90 muga.f90 simulatu.f90
#ifort -o marraztu tipos.f90 parametroak.f90 marraztu.f90  #<---- gnuploten emaitz ez bukatuak marraztu nahi baditut
#ifort -o hazia tipos.f90 hazia.f90  #<---- experimentuetarako seed-a lortzeko
...
:wq
EOF
sed -i '/grep -v/a ./simulatu' bidali.sh   #<<<----- aldatu bidaliko den lana
#sed -i '/grep -v/a ./marraztu' bidali.sh   #<<<----- aldatu bidaliko den lana
#sed -i '/grep -v/a ./hazia' bidali.sh   #<<<----- aldatu bidaliko den lana
